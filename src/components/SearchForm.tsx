...
export function SearchForm({ initialParams, onSubmit, isLoading }: SearchFormProps) {
  ...
  const [destinationInput, setDestinationInput] = useState(initialParams.destination)
  const [originOpen, setOriginOpen] = useState(false)
  const [destinationOpen, setDestinationOpen] = useState(false)
  const [originQuery, setOriginQuery] = useState(initialParams.origin)
  const [destinationQuery, setDestinationQuery] = useState(initialParams.destination)
  const { data: originCities = [] } = useCitySearch(originQuery, CITY_SEARCH_ENABLED)
  const { data: destinationCities = [] } = useCitySearch(destinationQuery, CITY_SEARCH_ENABLED)

  useEffect(() => {
    setForm(initialParams)
    setOriginInput(initialParams.origin)
    setDestinationInput(initialParams.destination)
  }, [initialParams])
  ...
  const handlePassengerChange = (key: keyof SearchParams['passengerCounts'], delta: number) => {
    setForm((prev) => {
      ...
      return { ...prev, passengerCounts: nextCounts, passengers: Math.max(1, total) }
    })
  }
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: SearchParams = {
      ...
      passengers:
        form.passengerCounts.adults + form.passengerCounts.children + form.passengerCounts.infants || form.passengers,
  ...
  const resolveCode = (input: string, suggestions: { iataCode: string }[]) => {
    ...
    if (suggestions.length) return suggestions[0].iataCode?.toUpperCase() ?? trimmed.toUpperCase().slice(0, 3)
            ...
            <div className="absolute left-0 right-0 z-999 mt-2 rounded-xl border border-white/15 bg-slate-900/90 p-3 shadow-xl backdrop-blur">
              {(['adults', 'children', 'infants'] as const).map((key) => (
                <div key={key} className="flex items-center justify-between py-1.5">
                  <div>
                    <p className="m-0 text-sm font-semibold text-white">{key.charAt(0).toUpperCase() + key.slice(1)}</p>
                  ...
                  <div className="inline-flex items-center gap-3">
                    ...
                    <span className="text-white">{form.passengerCounts[key]}</span>
                    <button
                      type="button"
                      className="grid h-8 w-8 place-items-center rounded-full border border-white/20 bg-white/10 text-lg text-white"
                      onClick={() => handlePassengerChange(key, 1)}
